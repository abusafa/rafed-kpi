...
type EntityItems = Awaited<ReturnType<typeof getOrgEntitiesByTypeCode>>["items"];
type EntityRow = EntityItems[number];
...
function latestEntityValue(row: EntityRow): number | null {
...
function toReportRow(e: EntityRow, et: EntityType): ReportRow {
  ...
  return {
    ...
    owner: typeof (e as unknown as Record<string, unknown>).ownerName === "string"
      ? String((e as unknown as Record<string, unknown>).ownerName)
      : null,
  };
}
...
export default function ReportsPage() {
  ...
  const loadAll = useCallback(async () => {
    setLoadingData(true);
    try {
      const types = (await getMyOrganizationEntityTypes()) as EntityType[];
      setEntityTypes(types);
      const results = await Promise.all(
        types.map((et) =>
          getOrgEntitiesByTypeCode({ entityTypeCode: et.code, page: 1, pageSize: 500 })
  ...
  if (!user) {
    return (
      <div className="rounded-2xl border border-border bg-card p-8">
        ...
        <Link href={`/${locale}/auth/login`} className="mt-3 inline-flex text-sm font-semibold text-foreground underline">
          {t("goToSignIn")}
        </Link>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <PageHeader
          ...
          icon={<Icon name="tabler:table" className="h-5 w-5" />}
        />
