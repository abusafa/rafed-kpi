...
export default function EditEntityPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const [detailRes, ownerRows] = await Promise.all([
          getOrgEntityDetail({ entityId }),
          getOrgOwnerOptions(),
        ]);
        if (!mounted) return;
        ...
        if (detailRes?.entity) {
          const e = detailRes.entity;
          setTitle(String(e.title ?? ""));
          setTitleAr(e.titleAr ? String(e.titleAr) : "");
          setKey(e.key ? String(e.key) : "");
          setDescription(e.description ? String(e.description) : "");
          setDescriptionAr(e.descriptionAr ? String(e.descriptionAr) : "");
          setStatus((e.status as StatusCode) ?? "PLANNED");
          setOwnerUserId(e.ownerUserId ?? undefined);
          setPeriodType((e.periodType as PeriodTypeCode) ?? "NONE");
          ...
          if (e.variables && e.variables.length > 0) {
            setVariables(
              e.variables.map((v) => ({
                tempId: makeTempId(),
                code: String(v.code ?? ""),
                displayName: String(v.displayName ?? v.code ?? ""),
                nameAr: v.nameAr ? String(v.nameAr) : "",
                dataType: (v.dataType as VariableDataTypeCode) ?? "NUMBER",
  ...
  }, [entityId, loading, user, userRole]);

  const typeLabel = useMemo(() => {
    if (!detail?.entity?.orgEntityType) return entityTypeCode;
    return df(detail.entity.orgEntityType.name, detail.entity.orgEntityType.nameAr);
  }, [df, detail, entityTypeCode]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await updateOrgEntity({
        entityId,
        title: title.trim(),
        titleAr: titleAr.trim() ? titleAr.trim() : undefined,
        key: key.trim() ? key.trim() : undefined,
        description: description.trim() ? description.trim() : undefined,
        descriptionAr: descriptionAr.trim() ? descriptionAr.trim() : undefined,
        status,
        ownerUserId,
      ...
      if (!res.success) {
        setError(te(res.error) || res.error || t("failedToUpdate"));
        return;
      }

      router.push(`/${locale}/entities/${entityTypeCode}/${entityId}`);
      router.refresh();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      setError(msg);
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={tr("Edit", "تعديل") + " " + typeLabel}
        subtitle={title || detail.entity.title}
      />
